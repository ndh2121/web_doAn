...
export const getInfoAfterOrdered = expressAsyncHandler(async (req, res) => {
  console.log('getInfoAfterOrdered')
  ...
  if (updateOrder) {
    ...
    const orderGhn = {
      "payment_type_id": 2,
      "note": "TungTaotest abc",
      "from_phone":"0909999999",
      "required_note": "KHONGCHOXEMHANG",
      "return_name": "Tung Tao",
      "return_phone": "0909999999",
      "return_address": "số 3, xóm 1, Văn Giáp",
      "return_ward_name": "Văn Bình",
      "return_district_name": "Thường Tín",
      "return_province_name":"Hà Nội",
      "client_order_code": "",
      "to_name": req.body.name,
      "to_phone": req.body.phone,
      to_address: `${req.body.shippingAddress.province}, ${req.body.shippingAddress.district}, ${req.body.shippingAddress.ward}, ${req.body.shippingAddress.detail}`,
      "to_ward_name":req.body.shippingAddress.ward,
      "to_district_name": req.body.shippingAddress.district,
      "to_province_name": req.body.shippingAddress.province,
      "content": "ABCDEF",
      "weight": 200,
      "length": 15,
      "width": 15,
    ...
    try {
      ...
      res.status(201).send({ message: "get order created", order: createOrder });
    } catch (error) {
...
export const updateOrder = expressAsyncHandler(async (req, res) => {
  ...
  if (updateOrder) {
    ...
    const orderGhn = {
      "payment_type_id": 2,
      "note": "TungTaotest abc",
      "from_name":"Cửa hàng điện thoại Tùng Táo",
      "from_phone":"0909999999",
      "from_address":"số 3, xóm 1, Văn Giáp",
      "from_ward_name":"Văn Bình",
      "from_district_name":"Thường Tín",
...
export const ShippingProduct = expressAsyncHandler(async (req, res) => {
  ...
  // ProductModel.findByIdAndUpdate({_id: product._id}, {amount: 3})
  const product = order.map(async (order) => {
      await ProductModel.findOneAndUpdate(
        { _id: order._id },
        { $inc: { amount: -Number(order.qty) } },
    )
  })
  if (Order) {
    Order.status = status;
